...
Monitor.prototype._fetchDestinations = function (shard, callback) {
  ...
  // get already created mirrors for given shard
  Contact.find({ id: { $in: shardExistNodeIdsMirrors } }, (err, results) => {
    if (err) {
      return callback(err);
    }
    ...
    Mirror.find({ shardHash: shard.hash, contact: { $exists: true }, isEstablished: false })
      .populate({
        path: 'contact',
        match: {
...
Monitor.prototype._transferShard = function (shard, state, callback) {
  ...
  if (!destination) {
    this.actions.publishNewContractsForShard(shard.hash, (err) => {
      if (err) {
        log.warn('Error publishing new contracts for shard %s', shard.hash);
      } else {
