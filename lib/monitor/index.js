...
Monitor.prototype._fetchOtherDestinations = function (shard, callback) {
  ...
  async.waterfall([
    next => Pointer.findOne({ hash: shard.hash }).exec(next),
    (pointer) => next => Frame.findOne({ shards: pointer._id }).exec(next),
    (frame, next) => Pointer.find({ _id: { $in: frame.shards } }).exec(next),
    (pointers, next) => {
      const pointerHashes = pointers.map(x => x.hash)
      Mirror.find({ isEstablished: true, shardHash: { $in: pointerHashes } }).exec(next);
    },
    next => {
...
Monitor.prototype._removeFarmer = function (farmer) {
  ...
  async.parallel([
    ...
    if (err) {
      ...
      Contact.deleteOne({ _id: farmer }, (err, contact) => {
        if (err) {
...
Monitor.prototype.run = function () {
  ...
  const checkMirrors = (err) => {
    ...
    const { User, Shard } = this.storage.models;
