...
Monitor.prototype._fetchOtherDestinations = function (shard, callback) {
  this.storage.models.Pointer.findOne({ hash: shard.hash }).exec((err, pointer) => {
    if (err) {
      callback(err)
    } else {
      this.storage.models.Frame.findOne({ shards: pointer._id }).exec((err, frame) => {
        if (err) {
          callback(err)
        } else {
          this.storage.models.Pointer.find({ _id: { $in: frame.shards } }).exec((err, pointers) => {
            if (err) {
              callback(err)
            } else {
              const pointerHashes = pointers.map(x => x.hash)
              this.storage.models.Mirror.find({ isEstablished: true, shardHash: { $in: pointerHashes } }).exec((err, mirrors) => {
                if (err) {
                  callback(err)
                } else {
                  const nodeIds = mirrors.map(x => x.contact)
                  callback(null, nodeIds)
                }
              })
            }
          })
  ...
  this.storage.models.Mirror
    ...
    .exec((err, results) => {
      ...
      const mirrors = results.filter((m) => {
        if (!m.contact) {
          log.warn('Monitor: Mirror %s is missing contact in database. Mirror destroyed.', m._id);
          m.remove()
          return false;
        } else if (shard.contracts[m.contact._id]) {
          // log.warn('Shard %s already established to contact %s', shard.hash, m.contact._id);
          // TODO: Set mirror to Established state
          // Or maybe contact doesn't have that shard
          return false;
        }
        return false;
      });
    ...
    .on('data', (data) => {
      ...
      this._replicateShard(shard, (err) => {
        if (err) {
          ...
          // remove ${contact} from shard ${shard}
...
Monitor.prototype.run = function () {
  ...
  const checkMirrors = (err) => {
    ...
    async.parallel([
      next => {
        utils.AggregationCursor(User, premiumMirrorsQuery, (data, nextData) => {
          log.info('Checking premium shard %s...', data._id);
          Shard.findOne({ hash: data._id }, (err, shardModel) => {
            if (err || !shardModel) {
            ...
            this._replicateShard(shardObject, function (err) {
              if (err) { log.error('Unable to replicate shard %s, reason: %s', shardModel.hash, err.message) }
              nextData(null, data);
            });
          })
        }, (err) => { next(err) })
      ...
      next => {
        utils.AggregationCursor(Shard, mirrorsQuery, (data, nextData) => {
