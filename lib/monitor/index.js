...
Monitor.prototype._fetchDestinations = function (shard, callback) {
  // StorageItem.prototype.addContract create shard contracts object with keys as nodeId(s)
  const shardExistNodeIdsMirrors = Object.keys(shard.contracts).filter(utils.distinct);
  let shardExistNodesMirrors = [];
  // get already created mirrors for given shard (for excluding destinations for those ecosystems on future shard mirror)
  this.storage.models.Contact
    .find({ id: { $in: shardExistNodeIdsMirrors }})
    .exec((err, results) => {
      if (err) {
        return callback(err);
      }
      ...
      if (results && results.length) {
        shardExistNodesMirrors = results;
      }
      ...
      this.storage.models.Mirror
        ...
        .exec((err, results) => {
          ...
          const shardExistNodesMirrorsIp = shardExistNodesMirrors.map(contact => contact.address);
          const mirrors = results.filter((m) => {
            if (!m.contact) {
              log.warn('Mirror %s is missing contact in database', m._id);
              return false;
            } else if(!m.contact.address) {
              log.warn('Mirror %s doesn\'t have a referenced node address', m._id, );
              return false;
            } else if(shardExistNodesMirrorsIp.includes(m.contact.address)) {
              log.warn('Mirror %s is not available as its address %s already has a Node with a mirror for this shard ', m._id, m.contact.address);
              return false;
...
Monitor.prototype._removeFarmer = function (farmer) {
  async.parallel([
    (callback) => {
      this.storage.models.Shard.update({
        $or: [
    ...
    (callback) => {
      this.storage.models.Mirror.deleteMany({ contact: farmer }, (err, total) => {
        if (err) {
    ...
    if (err) {
      ...
      this.storage.models.Contact.deleteOne({ _id: farmer }, (err, contact) => {
        if (err) {
...
Monitor.prototype._replicateFarmer = function (contact) {
  ...
  cursor
    ...
    .on('end', () => {
      ...
      if (!this._underAttack || replicationErrors == 0) {
        this._removeFarmer(contact.nodeID);
      } else {
        log.info('No need to remove farmer, reason: %s', (this._underAttack ? 'Under attack or CPU max.' : 'Replication errors'));
      }
    });
};
...
Monitor.prototype._DDosAttackCheker = function () {
  const trigger = 5.0;
  const load = parseFloat(require('fs').readFileSync('/proc/loadavg', 'utf8').trim().split(' ')[0]);
  this._underAttack = load > trigger;

  if (this._underAttack) {
    log.warn('Server overloaded. Preventing from remove farmers');
  }
}
...
Monitor.prototype.run = function () {
  ...
  const finish = (err) => {
    ...
    //if the rate of failures is greater than 5%, let's check if we are under attack.
    if (fail > 5) {
      this._DDosAttackCheker();
    }
    this._running = false;
    this.wait();
  };

  const checkMirrors = (err) => {
    log.info('START checking shards with less mirrors')
    if (err) {
      return finish(err)
    }
    ...
    const mirrorsQuery = [
      { '$project': { '_id': '$hash', 'total': { '$size': '$contracts' } } },
      { '$match': { 'total': { '$lt': 6 } } },
      { '$sort': { 'total': 1 } },
      { '$sample': { 'size': 50 } }
    ]
    ...
    async.waterfall([
      next => {
        // Get 50 mirrors from suscribed users
        User.aggregate(premiumMirrorsQuery).cursor({ batchSize: 1000 }).exec().toArray().then(mirrors => {
          const shardIds = mirrors.map((curr) => curr._id)
          next(null, shardIds)
        }).catch(next)
      },
      (premiumShardIds, next) => {
        // Get 50 mirrors from any other user
        Shard.aggregate(mirrorsQuery).cursor({ batchSize: 1000 }).exec().toArray().then(mirrors => {
          const shardIds = mirrors.map((curr) => curr._id)
          next(null, { premium: premiumShardIds, regular: shardIds })
        });
      },
      (shardIds, next) => {
        next(null, shardIds.premium.concat(shardIds.regular))
      },
      (shardIds, next) => {
        const cursor = Shard.find({ hash: { $in: shardIds } }).cursor()

        cursor.on('error', (err) => {
          log.error('Error quering shards to replicate');
          next(err)
        }).on('data', (data) => {
          cursor.pause();
          const shard = storj.StorageItem(data.toObject());
          log.info('Replicating shard %s', shard.hash);
          this._replicateShard(shard, (err) => {
            if (err) { log.error('Unable to replicate shard %s, reason: %s', shard.hash, err.message) }
            cursor.resume();
          });
        }).on('end', () => next());
