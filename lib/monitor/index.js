...
Monitor.prototype.run = function () {
  ...
  const checkMirrors = (err) => {
    ...
    const mirrorCursor = Mirror.aggregate(mirrorsQuery).cursor({ batchSize: 1000 }).exec()

    console.log(mirrorCursor.eachAsync)

    finish()

    /*
    mirrorCursor.eachAsync(async mirror => {
      const shardHash = mirror._id
      log.warn('Replicating shard %s', shardHash);

      await new Promise((resolve, reject) => {
        this.replicate.triggerMirrorEstablish(6, shardHash, (err) => {
          if (err) {
            log.error('Cannot replicate shard %s, reason: %s', shardHash, err.message)
            reject(err)
          } else {
            resolve()
          }
