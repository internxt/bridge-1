...
class Monitor {
  ...
  _fullyRemoveShard(hash) {
    ...
    return async.waterfall([
      (next) => Mirror.deleteMany({ shardHash: hash }).then(() => next()).catch(next),
      (next) => Shard.deleteOne({ hash: hash }).then(() => next()).catch(next),
      (next) => Pointer.deleteMany({ hash: hash }).then(() => next()).catch(next)
    ]);
  }
