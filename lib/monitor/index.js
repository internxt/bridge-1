...
class Monitor {
      ...
      .exec((err, results) => {
        ...
        const mirrors = results.filter((m) => {
          if (!m.contact) {
            ...
            Mirror.updateOne({ shardhash: shard.hash }, { isEstablished: true });
  ...
  _fetchSources(shard, callback) {
    ...
    Contact.find({ _id: { $in: farmers } }).sort({ lastSeen: -1 })
      .exec((err, results) => {
        if (err) {
          return callback(err);
        }
  ...
  _fullyRemoveShard(hash) {
    const { Pointer, Shard, Mirror } = this.storage.models;

    return async.waterfall([
      (next) => Mirror.deleteMany({ shardHash: hash }).then((x) => {
        // console.log('Removed %s mirrors, hash: %s', x.deletedCount, hash);
        next();
      }).catch(next),
      (next) => Shard.deleteOne({ hash: hash }).then((x) => {
        // console.log('Removed %s shards, hash: %s', x.deletedCount, hash);
        next();
      }).catch(next),
      (next) => Pointer.deleteMany({ hash: hash }).then((x) => {
        // console.log('Removed %s pointers, hash: %s', x.deletedCount, hash);
        next();
      }).catch(next)
    ]);
  }
  ...
  _replicateShard(shard, callback) {
    async.parallel({
      ...
      if (state.sources.length === 0) {
        // this._fullyRemoveShard(shard.hash).finally(() => callback(Error('Missing shard sources')));

        callback(Error('Missing shard sources'));
      } else if (state.sources.length > constants.M_REPLICATE - 1) {
  ...
  _removeFarmer(farmer) {
    log.info('Removing farmer %s', farmer);
    const { Contact, Mirror } = this.storage.models;

    return async.waterfall([
      // (next) => Contact.deleteOne({ _id: farmer }).then(() => next()).catch(next),
  ...
  _replicateFarmer(contact, callback) {
    ...
    utils.AggregationCursor(Shard, query, (data, nextData) => {
      const shard = storj.StorageItem(data);
      ...
      this._replicateShard(shard, (err) => {
        ...
        this._removeFarmerFromShard(shard.hash, contact.nodeID).finally(() => nextData(null, data));
      });
    }, (err) => {
      if (err) {
      ...
      async.eachLimit(contacts, pingConcurrency, (contactData, next) => {
        ...
        this.network.ping(contact, (err) => {
          if (err) {
            ...
            contactData.recordTimeoutFailure().save((err) => {
              ...
              Contact.findOne({ _id: contact.nodeID }).then(contact2 => {
                if (contact2.timeoutRate >= timeoutRateThreshold) {
                  log.warn('Shards from farmer %s must be replicated, timeoutRate: %s', contact2.nodeID, contact2.timeoutRate);
                  this._replicateFarmer(contact, () => next());
                } else {
            ...
            log.info('PING to %s OK: Node is online. Update timeoutRate to 0', contact.nodeID);
