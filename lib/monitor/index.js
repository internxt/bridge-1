...
Monitor.prototype.run = function () {
  ...
  const checkMirrors = (err) => {
    ...
    Mirror.aggregate(mirrorsQuery).cursor({ batchSize: 1000 }).exec().toArray().then(mirrors => {
      ...
      Shard.find({ hash: { $in: shardIds } }).exec((err, results) => {
        if (err) {
          log.error('Failed to find shards to replicate')
          finish(err)
        } else {
          async.forEach(results, (shard, next) => {
            this._replicateShard(shard, (err) => {
              if (err) {
                log.warn('Error replicating shard %s: %s', shard.hash, err.message)
              }
              next()
            })
          }, (err) => finish(err))
