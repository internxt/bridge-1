...
Monitor.prototype._fetchDestinations = function (shard, callback) {
  ...
  this.storage.models.Contact
    ...
    .exec((err, results) => {
      ...
      const shardExistNodesMirrorsAddrs = shardExistNodesMirrors.map(contact => contact.address);

      this.storage.models.Mirror
        .find({ shardHash: shard.hash, contact: { $exists: true }, isEstablished: true })
        .populate({
          path: 'contact',
          match: {
            _id: { $nin: shardExistNodeIdsMirrors },
            address: { $nin: shardExistNodesMirrorsAddrs, $exists: true }
          }
        })
        .exec((err, results) => {
          if (err) {
