...
Monitor.prototype.run = function () {
  ...
  const checkMirrors = (err) => {
    ...
    const mirrorsQuery = [
      { '$group': { '_id': '$shardHash', 'total': { '$sum': 1 } } },
      { '$match': { 'total': { '$lt': constants.M_REPLICATE } } },
      { '$sort': { 'total': 1 } },
      { '$sample': { 'size': 50 } }
    ]
    ...
    const premiumMirrorsQuery = [
      ...
      { '$unwind': { 'path': '$shard' } },
      {
        ...
        '$group': { '_id': '$mirror.shardHash', 'total': { '$sum': 1 } }
      },
      { '$match': { 'total': { '$lt': constants.M_REPLICATE } } },
      { '$sort': { 'total': 1 } },
      { '$sample': { size: 50 } }
    ...
    async.waterfall([
      next => {
        ...
        Mirror.aggregate(premiumMirrorsQuery).cursor({ batchSize: 1000 }).exec().toArray().then(mirrors => {
      ...
      (premiumShardIds, next) => {
        ...
        Mirror.aggregate(mirrorsQuery).cursor({ batchSize: 1000 }).exec().toArray().then(mirrors => {
          const shardIds = mirrors.map((curr) => curr._id)
