        ...
        return utils.AggregationCursor(User, premiumMirrorsQuery, (data, nextData) => {
          ...
          return Shard.findOne({ hash: data._id }, (err, shardModel) => {
            ...
            return this._replicateShard(shardObject, function (err) {
              if (err) {
                log.error('Unable to replicate premium shard %s, reason: %s', shardModel.hash, err.message);
              }
              nextData(null, data);
            });
          });
      ...
      next => {
        return utils.AggregationCursor(User, mirrorsQuery, (data, nextData) => {
          ...
          return Shard.findOne({ hash: data._id }, (err, shardModel) => {
            ...
            return this._replicateShard(shardObject, function (err) {
              if (err) {
                log.error('Unable to replicate regular shard %s, reason: %s', shardModel.hash, err.message);
              }
              nextData(null, data);
            });
          });
      ...
      next => {
        return utils.AggregationCursor(Mirror, recentMirrorsCreated, (data, nextData) => {
          ...
          return Shard.findOne({ hash: data._id }, (err, shardModel) => {
            ...
            return this._replicateShard(shardObject, function (err) {
              if (err) {
                log.error('Unable to replicate recent shard %s, reason: %s', shardModel.hash, err.message);
              }
              nextData(null, data);
            });
          });
