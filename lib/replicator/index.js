...
class Replicator {
  ...
  _randomTime(max, min) {
    // TODO: Export this function into a common monitor's utilities file
    const range = max - min;
  ...
  checkMirrors() {
    ...
    async.parallel([
      ...
      next => {
        return utils.AggregationCursor(Shard, mirrorsQuery, (data, nextData) => {
    ...
    this.storage.models.Mirror
      ...
      .exec((err, results) => {
        ...
        const mirrors = results.filter((m) => {
          if (!m.contact) {
            log.warn('Replicator: Mirror %s is missing contact in database. Mirror destroyed.', m._id);
            m.remove();
            return false;
  ...
  wait() {
    ...
    this._timeout = setTimeout(() => this.run(), 5000);
  }
