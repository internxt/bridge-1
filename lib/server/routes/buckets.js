...
BucketsRouter.prototype.destroyBucketById = function (req, res, next) {
  ...
  log.warn('destroyBucketById: User %s requested remove bucket %s', req.user._id, req.params.id);
  ...
  BucketEntry.aggregate([
    ...
    .exec().toArray().then(storageEvents => {
      ...
      Bucket.findOne({
        ...
        if (err) {
          log.error('destroyBucketById: Error performing query for bucket %s, reason %s', req.params.id, err.message);
          return next(new errors.InternalError(err.message));
        }

        if (!bucket) {
          log.error('destroyBucketById: Bucket not found %s', req.params.id);
          return next(new errors.NotFoundError('Bucket not found'));
        }

        bucket.remove(err => {
          if (err) {
            log.error('destroyBucketById: Error removing bucket %s', req.params.id);
            return next(new errors.InternalError(err.message));
          }

          BucketEntry.remove({ bucket: req.params.id }, err => {
            if (err) {
              log.error('destroyBucketById: Unable to remove bucket entries from %s, reason: %s',
                req.params.id, err.message);
            }

            StorageEvent.collection.insert(storageEvents, err => {
              if (err) {
                log.warn('destroyBucketById: Unable to save storage events from %s, reason: %s',
                  req.params.id, err.message);
              } else {
                log.warn('destroyBucketById: Bucket destroyed succesfully. %s', req.params.id);
              }
            });
            res.status(204).end();
          });
      ...
      log.error('destroyBucketById: Failed to query BucketEntries of %s, reason: %s', req.params.id, err.message);
      res.status(204).end();
    });
...
BucketsRouter.prototype.getStorageUsage = function (req, res, next) {
