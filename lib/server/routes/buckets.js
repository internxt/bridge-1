...
BucketsRouter.prototype._getPointersFromEntry = function (entry, opts, user, done) {
  ...
  cursor.exec(function (err, pointers) {
    ...
    if (!Number.isFinite(bytes)) {
      ...
      return done(new errors.InternalError('Pointer size in not a finite number'));
    }
...
BucketsRouter.prototype.getFile2 = function (req, res, next) {
  const self = this;
  ...
  Bucket.findOne({
    ...
    BucketEntry.findOne({
      ...
      entry.remove(function (err) {
        ...
        // TODO: Use async.waterfall to control the sequence flow
        Frame.findOne({ _id: entry.frame.id }, (err, frame) => {
          if (err) {
            return log.error('Cannot find frame %s from bucket %s, reason: %s', entry.frame.id, entry.id, err.message);
          }
          Pointer.find({ _id: { $in: frame.shards } }, (err, pointers) => {
            ...
            async.eachSeries(pointers, (pointer, nextPointer) => {
              // renewal.invalidateContract(pointer.hash, () => { });
              pointer.remove((err) => {
                if (err) {
                ...
                frame.remove((err) => {
                  if (err) {
                    log.error('Error deleting frame %s, reason: %s', frame._id, err.message);
                  }
                });
              });
            });
...
BucketsRouter.prototype._definitions = function () {
  ...
  return [
    ...
    ['GET', '/buckets2/:id/files/:file', this.getLimiter(limiter(1000)), this._validate, this._usetokenOrVerify, this.getFile2],
