        ...
        stripe.products.retrieve(productId, (err, product) => {
            ...
            User.findOne({ _id: email }, (err, user) => {
                if (err || !user) {
                    ...
                    if (planSize === 2147483648) { planInfo.name = 'Free 2GB'; }
                    ...
                    if (data.livemode) {
                        this.analytics.track({
                            anonymousId: user.uuid, event: 'subscription-trial-start',
                            properties: {
                                email: email,
...
StripeRouter.prototype._webhookCustomerSubscriptionDeleted = function (data, res, next) {
    ...
    stripe.customers.retrieve(customer, (err, customer_obj) => {
        ...
        User.findOne({ _id: email }, (err, user) => {
            ...
            if (data.livemode) {
                this.analytics.track({ anonymousId: user.uuid, event: 'subscription-cancelled', properties: { email: email, date: (new Date()).toISOString() } })
            }
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = function (data, res, next) {
    ...
    stripe.customers.retrieve(customer, (err, customer_obj) => {
        ...
        if (data.livemode) {
            ...
            User.findOne({ _id: email }, (err, user) => {
                if (!err && user) {
                    this.analytics.track({ userId: user.uuid, event: 'subscription-renewed', properties: { email: email, date: (new Date()).toISOString() } })
...
StripeRouter.prototype._webhookPaymentIntentFailed = function (data, res, next) {
    ...
    stripe.customers.retrieve(customer, (err, customer_obj) => {
        ...
        User.findOne({ _id: email }, (err, user) => {
            ...
            if (!err && user) {
                this.analytics.track({ userId: user.uuid, event: 'subscription-renewal-failed', properties: { email: email, date: (new Date()).toISOString() } })
            }
        })
        res.status(200).send();
    });
