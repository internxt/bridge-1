...
function getLifetimeTier(tier) {
  switch (tier) {
    case 'lifetime':
      // lifetime_tier = 2tb
      return '2TB';
    case 'exclusive-lifetime':
      // lifetime_tier = exclusive
      return '10TB';
    case 'infinite':
      // infinite
      return 'infinite';
...
StripeRouter.prototype._webhookCheckoutSessionCompleted = async function (data, res) {
  ...
  if (!object.display_items) {
    return res.status(200).send({ info: `Possible lifetime payment by link ${object.customer_details.email}` });
  }
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  ...
  if (metadata.member_tier === 'lifetime') {
    ...
    // call to get lifetime tier
    const lifetime_tier = getLifetimeTier(metadata.lifetime_tier);
    if (!lifetime_tier) {
      log.error('Error when creating lifetime.No tier specified.')
      return res.status(500).send({ info: 'Error when creating lifetime. No tier specified.'})
    }
    if (!user) {
      try {
