...
StripeRouter.prototype._updateToLifetimeAccount = async function (email, lifetime_tier, analytics_info) {
  ...
  else return Promise.reject('Could not update the user plan to lifetime.');
};

StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  const User = this.storage.models.User;

  const metadata = data.data.object.charges.data[0].metadata;

  const email = data.data.object.charges.data[0].billing_details.email;

  let user = await User.findOne({ _id: email.toLowerCase() });
  ...
  if (metadata.member_tier === 'lifetime') {
    ...
    if (!user) {
      await this._createAccount(email.toLowerCase(), 'lifetime_' + lifetime_tier);
    }
    ...
    await this._updateToLifetimeAccount(email.toLowerCase(), lifetime_tier, analytics_info).catch(msg => { return res.status(500, { msg: msg });});
    // update the user account if already has one
