...
function StripeRouter(options) {
  ...
  this.checkIfWebhookAlreadyFired = async function(event) {
    const stripe = this._getStripe(!data.livemode);
    const { data } = await stripe.events.list({
      delivery_success: false,
    });
    return data.filter(pending => pending.id === event.id && pending.type === event.type).length != 0
  }
}
...
StripeRouter.prototype._getStripe = function (isTest = false) {
  const stripe = new Stripe(isTest ? this.config.stripe.SK_TEST : this.config.stripe.SK_LIVE, { apiVersion: '2020-08-27' });
  return stripe;
};
...
StripeRouter.prototype._webhookCheckoutSessionCompleted = async function (data, res, next) {
  const User = this.storage.models.User;
  ...
  stripe.customers.retrieve(customer, (err, customer_obj) => {
    if(err || !customer_object || !customer_object.email){
    ...
    stripe.products.retrieve(productId, (err, product) => {
      ...
      User.findOne({ _id: email }, (err, user) => {
        if (err || !user) {
          ...
          if (planSize === 1024 * 1024 * 1024 * 10 || planSize === 0) { planInfo.name = '10GB'; }
          else if (planSize === 1024 * 1024 * 1024 * 3) { planInfo.name = '3GB'; }
          else if (planSize === 1024 * 1024 * 1024 * 20) { planInfo.name = '20GB'; }
          else if (planSize === 1024 * 1024 * 1024 * 200) { planInfo.name = '200GB'; }
          else if (planSize === 1024 * 1024 * 1024 * 1024 * 2) { planInfo.name = '2TB'; }
          else {
          ...
          user.save(err => {
            if (!err) {
              ...
              if(!data.livemode) {
                this.checkIfWebhookAlreadyFired(data).then( (isFired) => {
                  if(!isFired) {
          ...
          if (data.livemode) {
            this.checkIfWebhookAlreadyFired(data.id, data.type).then((isFired) => {
              if (!isFired) {
                this.analytics.identify({
                  userId: user.uuid,
                  traits: {
                    ...
                    member_tier: 'trial',
                    plan: planInfo.name,
                    payment_frequency: planInfo.interval_count + '' + planInfo.interval,
                ...
                return res.status(200).send({ msg: 'This webhook has already been sent' })
              }
            }
          }
        }
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res, next) {
  ...
  const paymentIntentId = data.data.object.id;
  const paymentIntent = await stripe.paymentIntents.retrieve(paymentIntentId);
  const metadata = paymentIntent.metadata;

  const customer_obj = await stripe.customers.retrieve(customerId);
  const email = customer_obj.email;

  const user = await User.findOne({ _id: email.toLowerCase() });

  if (metadata.member_tier === 'lifetime') {
    console.log('PAYMENT INTENT LIFETIME');
    // SHOULD CREATE ACCOUNT IF DON'T EXISTS

    if (!user) {
      await this._createAccount(email.toLowerCase(), 'lifetime_2TB');
    }
...
StripeRouter.prototype._webhookPaymentIntentCanceled = function (data, res, next) {
