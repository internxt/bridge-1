...
function StripeRouter(options) {
  ...
  const segmentKey = process.env.NODE_ENV === 'production' ? options.config.api_keys.segment : options.config.api_keys.segment_test;
  this.analytics = new Analytics(segmentKey, { flushAt: 1 });
}
  ...
  if (metadata.member_tier === 'lifetime') {
    ...
    if (!user) {
      try {
        ...
        return res.status(500).send({ info: 'Error create Account', msg: e });
      }
    }
    ...
    try {
      await this._updateToLifetimeAccount(email, lifetime_tier, analytics_info);
    } catch (err) {
      log.error('Error when upgrading to Lifetime. ' + err);

      return res.status(500).send({ info: 'Error when upgrading', msg: err });
    }
...
StripeRouter.prototype.parseWebhook = function (req, res, next) {
  ...
  if (process.env.NODE_ENV === 'production' && isLiveMode) {
    ...
    webhookObject = JSON.parse(req.rawbody);
  }
