    ...
    if(!object.livemode) {
        ...
        User.findOne({ _id: email}, (err, user) => {
            if(err || !user) {
                ...
                const productId = object.lines.data[0].plan.product;
                stripe.products.retrieve(productId, (err, product) => {
                    if(err) {
                        ...
                        return res.status(500).send({ error: 'Error retrieving product' });
                    }
                    else {
                        const metadata = product.metadata;
                        // Ensure that we have defined the necessary properties for product metadata
                        if (metadata.member_tier && metadata.simple_plan) {
                            ...
                            this.analytics.identify({
                                ...
                                traits: {
                                    ...
                                    payment_frequency: object.lines.data[0].plan.interval,
                                    email: email,
                                    date: (new Date()).toISOString(),
                                    coupon: object.discount ? object.discount.coupon.name : undefined,
                                    plan: metadata.simple_plan
                                }
                            }, (err) => {
                                if (err) return res.status(500).send({ error: 'Identify event was not sent.' });
                                else {
                                    this.analytics.track({
                                        ...
                                        if (err) return res.status(500).send({ error: 'Identify event was not sent.' });
                                        else res.status(200).send(`${metadata.member_tier} payment tracked`);
                                    });
                                }
                            });
                        }
                        else {
                            log.error('Webhook error missing product metadata: member_tier and simple_plan traits');
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res, next) {
    ...
    const object = data.data.object;
    const customer = object.customer;
    const charge = object.charges.data[0]; // At this moment, we only track 1 payment
    // Only lifetime subscriptions have an invoice before the payment is
