...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  ...
  if (metadata.member_tier === 'lifetime') {
    ...
    if (!user) {
      await this._createAccount(email, 'lifetime_' + lifetime_tier).catch(msg => {
        log.error(`Error Creating Lifetime User. Email: ${email}, Tier: ${lifetime_tier} . Reason: ${msg}`);

        return res.status(500).send({ msg: `Error Creating Lifetime User. Email: ${email}, Tier: ${lifetime_tier} . Reason: ${msg}` });
      });
    }
    ...
    await this._updateToLifetimeAccount(email, lifetime_tier, analytics_info).catch(msg => {
      log.error(`Error Upgrading User in database to Lifetime Account. Tier ${lifetime_tier}, Email: ${email}  Reason: {msg}`);
