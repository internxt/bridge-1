  ...
  stripeUtils.customerExists(req.user._id).then(customer => {
    ...
    if (!customer) {
      ...
      stripeUtils.createNewCustomer(req.user._id, token).then(newCustomer => {
        ...
        stripeUtils.subscribeCustomerToPlan(newCustomer.id, req.params.plan_id).then(() => {
      ...
      if (subscriptions.length > 0) {
        ...
        if (current_plan_info.index === 2) {
          ...
          if (current_plan_info.index !== '0') {
            stripeUtils.cancelSubscriptionById(subscription_id).then(() => {
              console.log('Unsubscribed user: ' + current_plan_info.plan_name);

              stripeUtils.subscribeCustomerToPlan(customer.id, req.params.plan).then(() => {
        ...
        stripeUtils.subscribeCustomerToPlan(customer.data[0].id, req.params.plan_id).then(() => {
          console.log('SUBSCRIPTION succesful');
