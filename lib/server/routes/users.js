...
UsersRouter.prototype.createUser = function (req, res, next) {
  ...
  if (!match) { return res.status(500).send({ error: 'Not valid email address' }) }
  ...
  if (!allowedEmail || disposableGmailPattern.test(req.body.email)) { return res.status(500).send({ error: 'Not valid email address provider' }) }
