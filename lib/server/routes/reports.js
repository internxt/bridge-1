  ...
  this.storage.models.Shard.find({
    ...
    self._handleExchangeReport(exchangeReport, (err) => {
      ...
      if (err) {
        ...
        if (exchangeReport.exchangeResultMessage == 'MIRROR_FAILED' || exchangeReport.exchangeResultMessage == 'AUDIT_FAILED') {
          self.storage.models.Mirror.deleteOne({
            ...
            self.monitor.actions.publishNewContractsForShard(exchangeReport.dataHash, (err) => {
              log.info('New mirrors created due to a failed mirror');
            });
          });
        }
        ...
        switch (err.message) {
          case 'Auto mirroring limit is reached':
            break;
          default:
            return log.warn('Exange Report error: %s', err.message);
        }
      }
    });
