      ...
      if (err) {
        ...
        switch (err.message) {
          case 'Auto mirroring limit is reached':
            break;
          default:
            return log.warn('Exchange Report Error: %s', err.message);
        }
      }
    });
...
ReportsRouter.prototype._handleExchangeReport = function (report, callback) {
  ...
  switch (exchangeResultMessage) {
    ...
    case 'SHARD_DOWNLOADED':
      this._shardHasMirrors(dataHash, (err, results) => {
        if (err) {

        } else {
          if (results.established < constants.M_REPLICATE) {
            if (report.clientId) {
              // console.log('User %s, shard %s doesn\'t have enough mirrors: %s', report.clientId, dataHash, results.established)
              this._triggerMirrorEstablish(constants.M_REPLICATE, dataHash, callback);
            }
          }
        }
