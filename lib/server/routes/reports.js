 ...
 * @param {Function} next
 */
ReportsRouter.prototype.createExchangeReport = async function (req, res, next) {
  const self = this;
  const { ExchangeReport, Shard, Contact } = this.storage.models;
  var exchangeReport = new ExchangeReport(req.body);
  var projection = { hash: true, contracts: true };

  const contact = await Contact.findOne({ _id: exchangeReport.farmerId });

  if (!contact) {
    console.log(exchangeReport.farmerId);
  }

  if (contact && contact.timeoutRate >= 0.12) {
    return next(new errors.BadRequestError('Invalid farmer'));
  }
...
ReportsRouter.prototype.getMirrorsListByHash = function () {
