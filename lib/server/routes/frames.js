...
FramesRouter.prototype.getStorageUsage = function (req, res, next) {
  ...
  Frame.find({ user: req.user._id }).exec(function (err, frames) {
    if (err) {
      return next(new errors.InternalError(err.message));
    }
    ...
    var finalObject = frames.map(frame => {
      return frame.toObject().storageSize;
    });

    var sum = finalObject.reduce((a, b) => a + b, 0);
