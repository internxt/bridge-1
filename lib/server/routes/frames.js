...
FramesRouter.prototype.addShardToFrame = function (req, res, next) {
    ...
    getStorageLimit(req.user._id).then(limit => {
      if (!limit) {
        limit = 1;
      }
      ...
      getStorageUsage(this.storage, req.user._id).then(usage => {
        if (!usage) {
          usage = { total: 0 };
        }
        ...
        if (usage.total < limit) {
          this.addShardToFrame2(req, res, next);
        } else {
          log.warn('USER LIMIT REACHED FOR ' + req.user.email + ' : ' + usage.total + ' of ' + limit);
          return next(new errors.TransferRateError(
            'Could not add shard to frame, transfer limit reached. Upgrade your plan.'
          ));
        }
      });
    });
 ...
 * @param {Function} next
 */
FramesRouter.prototype.addShardToFrame2 = function (req, res, next) {
...
function getStorageLimit(user) {
  ...
  return new Promise((resolve, reject) => {
    try {
      conn.query('select T2.space_gb from users T0 left join subscriptions T1 on T0.id = T1.user join plans T2 on T1.plan = T2.id where T0.email = ?', [user], function (err, results) {
        if (err) {
          resolve(1);
        } else {
          try {
            resolve(results[0].space_gb);
          } catch (err2) {
            resolve(1);
          }
        }
...
FramesRouter.prototype.getStorageLimit = function (req, res, next) {
  ...
  .then(limit => {
    ...
    res.status(200).send({ total: limit });
  }).catch(err => {
    res.status(400).send({ message: 'Error', reason: err.message });
  });
}
