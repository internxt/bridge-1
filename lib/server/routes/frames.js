...
FramesRouter.prototype._selectFarmers = function (excluded, callback) {
  ...
  const xNodes = constants.XNODES;
  const randomNodeID = crypto.randomBytes(20).toString('hex');

  async.parallel([
    (next) => {
      Contact.find({ _id: { $in: xNodes } }).sort({ _id: -1 }).exec(next);
    },
    (next) => {
      Contact.find({
        _id: {
        ...
        timeoutRate: { $lt: this._defaults.timeoutRateThreshold },
        spaceAvailable: true
      }).sort({ _id: -1 })
        .limit(this._defaults.publishTotal).exec(next);
    },
    (next) => {
      Contact.find({
        _id: {
        ...
        .limit(this._defaults.publishBenchTotal).exec(next);
    }
  ], (err, results) => {
    if (err) {
      return callback(err);
    ...
    function getContract(done) {
      ...
      Mirror.find({
        ...
        if (mirrors && mirrors.length) {
          ...
          uniqIps.sort(utils.sortByReputation);
