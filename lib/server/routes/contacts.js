...
function checkForDuplicateContactAddress (model, address) {
  if (isValidIp(address)) {
    return new Promise((resolve, reject) => {
      log.info('DEBUG201 | Checking for duplicate address')
      return model.findOne({ address }, function(err, doc) {
        if (doc) {
          log.info('DEBUG202 | Duplicate contact found')
          return true
        }
      })
    });
  } else {
    const contactAddress = getHostName(address)
    return new Promise((resolve, reject) => {
      log.info('DEBUG203 | Checking for duplicate host')
      return model.find({ address: { "$regex": contactAddress, "$options": "i" } }, function(err, doc) {
        if (doc) {
          log.info('DEBUG204 | Duplicate contact found')
          return true
        }
      })
    });
...
ContactsRouter.prototype.createContact = async function(req, res, next) {
  ...
  if (!isValidIp(req.body.address)) {
    return next(new Error('Not valid IP adress'))
  }

  if (await checkForDuplicateContactAddress(Contact, req.body.address)) {
    return next(new Error('Duplicate contact attempt'))
  }

  log.info(`DEBUG100 | Creating contact with address: ${req.body.address}`)
  Contact.record({
...
ContactsRouter.prototype.patchContactByNodeID = async function(req, res, next) {
  ...
  log.info(`DEBUG200 | Updating contact with address ${req.body.address}`)
