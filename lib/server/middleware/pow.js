...
function initTarget(db, callback) {
  ...
  async.series([
    (next) => db.hset('contact-stats', 'target', initialTarget, next),
    (next) => db.hset('contact-stats', 'timestamp', 0, next),
    (next) => db.hset('contact-stats', 'count', 0, next),
  ], callback);
}
...
function getTarget(db, opts, callback) {
  ...
  db.hgetall('contact-stats', function (err, stats) {
    ...
    if (!Number.isSafeInteger(parseInt(stats.count)) || !Number.isSafeInteger(parseInt(stats.timestamp))) {
    ...
    if (now > timestamp + retargetPeriod) {
      ...
      async.series([
        (next) => db.hset('contact-stats', 'target', newTarget, next),
        (next) => db.hset('contact-stats', 'timestamp', now, next),
        (next) => db.hset('contact-stats', 'count', 0, next),
      ], (err) => {
        if (err) {
          return callback(err);
...
function getChallenge(db, opts, callback) {
  module.exports.getTarget(db, opts, function (err, target) {
    ...
    db.set(key, target, 'EX', CHALLENGE_TTL_SECONDS, function (err) {
      ...
      callback(null, { challenge: challenge, target: target });
    });
  });
}
