...
Cleaner.prototype.clearBucketEntries = function (callback) {
  log.info('Check bucket entries of non existen buckets')
...
Cleaner.prototype.clearFramesWithoutUser = function (callback) {
  log.info('Check frames of non existen users')
...
Cleaner.prototype.clearPointers = function (callback) {
  log.info('Check pointers of non existen frames')
...
Cleaner.prototype.clearShardsWithoutPointers = function (callback) {
  log.info('Check shards of non existen pointers')
...
Cleaner.prototype.clearInvalidContracts = function (callback) {
  log.info('Check CONTRACTS')
  // Look for shards and inspect its contracts, delete those invalid contracts from shard
  const Shard = this.storage.models.Shard
  const query = { _id: { $gt: this.currentObjectId } }

  Shard.find(query).limit(100).exec((err, shards) => {
    if (err) {
      log.error(err.message)
    } else {
      async.forEach(shards, (shard, nextShard) => {
        console.log(shard)
        nextShard()
      }, (err) => {
        if (err) {
          log.error('Error clearing invalid contracts: %s', err.message)
        }
        callback(err)
      })
    }
...
Cleaner.prototype.clearShardsWithAllInvalidContracts = function (callback) {
  // Shards whose contracts are full of empty nodes. If only one node is valid, the shard won't be deleted.
  // TODO: If contracts.length !== validContracts.length --> DELETE THOSE PARTICULAR CONTRACTS in the shard, but not the shard itself
...
Cleaner.prototype.clearShardsWithoutContracts = function (callback) {
  ...
  // This rare case, a shard entry in the database with an empty the contracts array
...
Cleaner.prototype.wait = function () {
  ...
  const milliseconds = this._randomTime(10000, 5000);
