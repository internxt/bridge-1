...
Cleaner.prototype.clearBucketEntries = function (callback) {
  ...
  AggregationCursor(BucketEntry, query, (bucketEntry, nextBucketEntry) => {
    console.log(bucketEntry.destroy);
    // BucketEntry.destroy();
...
Cleaner.prototype.clearShardsWithoutPointers = function (callback) {
  ...
  return AggregationCursor(Shard, query, (shard, nextShard) => {
    console.log('Delete shard %s with hash %s', shard._id, shard.hash);
    async.parallel([
...
Cleaner.prototype.clearInvalidContracts = function (callback) {
  ...
  const cursor = Shard.find({
    _id: { $gt: this.currentObjectId }
  }).sort({ _id: 1 }).limit(1500).cursor();
  ...
  cursor.on('error', (err) => {
    ...
    async.eachSeries(contracts, (contract, nextContract) => {
      ...
      if (err) { log.error('Cannot end async contracts search', err); }
      cursor.resume();
    });
  }).on('end', () => {
