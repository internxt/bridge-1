...
Cleaner.prototype.clearShards = function (callback) {
  ...
  async.waterfall([
    ...
    this.clearShardsWithoutContacts.bind(this)
  ], (err) => callback(err))
}
...
Cleaner.prototype.clearShardsWithoutContacts = function (callback) {
  log.info('Clear shards without contacts')
  const Shard = this.storage.models.Shard
  const query = [
    {
      $match: {
