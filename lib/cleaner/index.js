...
const { AggregationCursor } = require('../utils');
...
function Cleaner(config) {
    ...
    this.currentObjectId = new ObjectId('5f5677be099ef66eed59b4e3');
...
Cleaner.prototype.start = function (callback) {
    ...
    this.client = Amplitude.init(this._config.amplitude.api_key);

    this.client.logEvent({
        event_type: 'Cleaner Event',
        user_id: 'bridge-01',
        ip: 'api.internxt.com',
        platform: 'Ubuntu 18',
        event_properties: {
...
Cleaner.prototype.clearFramesWithoutBucketEntries = function (callback) {
    ...
    const dayAgo = new Date(currentDate1.setDate(currentDate1.getDate() - 1));
    const currentDate2 = new Date();
    const weekAgo = new Date(currentDate2.setDate(currentDate2.getDate() - 30));

    // Frames within last 24h should not be deleted
    // We can assume > 1 day is a dead frame
    // Filter by last week to improve query speed
    const query = [
        {
            '$match': {
                '$and': [
                    { 'created': { '$gt': weekAgo.toISOString() } },
                    { 'created': { '$lt': dayAgo.toISOString() } }]
            }
        },
        {
            '$lookup': {
        ...
        { '$match': { 'bucketentry': { '$size': 0 } } }
    ];

    AggregationCursor(Frame, query, (data, nextData) => {
        if (data) { Frame.deleteOne({ _id: data._id }, (err) => nextData(err, data)); }
        else { nextData(); }
    }, (err) => callback(err));
};
...
Cleaner.prototype.clearFramesWithoutUser = function (callback) {
    ...
    const query = [
        { '$match': { 'created': { '$lt': dayAgo.toISOString() } } },
        {
            '$lookup': {
                'from': 'users',
                'localField': 'user',
...
Cleaner.prototype.clearShardsWithoutPointers = function (callback) {
    ...
    Shard.aggregate(query).cursor({ batchSize: 2500 }).exec().toArray().then(shards => {
        ...
        async.eachSeries(shardHashes, (shardHash, nextShardHash) => {
            this.renewal.invalidateContract(shardHash, () => { nextShardHash(); });
        }, () => {
            async.parallel([
                next => Shard.deleteMany({ _id: { $in: shardIds } }, (err) => next(err)),
                next => Mirror.deleteMany({ shardHash: { $in: shardHashes } }, (err) => next(err))
            ], (err) => callback(err))
        });
    }).catch(err => callback(err));
};
...
Cleaner.prototype.clearInvalidContracts = function (callback) {
    ...
    const { Shard, Contact, Mirror } = this.storage.models;

    const cursor = Shard.find({ _id: { $gt: this.currentObjectId } }).sort({ _id: 1 }).limit(1500).cursor();
                    ...
                    if (err) {
                        ...
                        if (!contact) {
                            async.parallel([
                                ...
                                next => Mirror.deleteMany({ shardHash: currentShardHash }, (err) => next(err))
                            ], (err) => {
                                nextContract();
                            })
                        } else {
...
Cleaner.prototype.clearShardsWithAllInvalidContracts = function (callback) {
    ...
    log.info('Check invalid contracts from shards (use sample 15000)');
    const { Shard, Mirror } = this.storage.models;
    const query = [
        { $sample: { size: 15000 } },
        {
