...
Cleaner.prototype.clearOldExchangeReports = function (callback) {
  ...
  ExchangeReport.deleteMany({ created: { $lt: NOW - ONEWEEK } }, (err, result) => {
    if (err) {
      callback(err);
    } else {
...
Cleaner.prototype.clearBuckets = function (callback) {
  ...
  AggregationCursor(Bucket, query, (bucket, nextBucket) => {
    Bucket.deleteOne({ _id: { $in: bucket._id } });
    nextBucket(null, bucket);
  }, (err) => callback(err));
};
...
Cleaner.prototype.clearBucketEntries = function (callback) {
  ...
  AggregationCursor(BucketEntry, query, (bucketEntry, nextBucketEntry) => {
    BucketEntry.deleteOne({ _id: { $in: bucketEntry._id } });
    nextBucketEntry(null, bucketEntry);
  }, (err) => callback(err));
};
  ...
  const query = [
    {
      '$match': {
        '$and': [
          {
            'created': { '$gt': new Date(new Date() - ms('14d')).toISOString() }
          },
          {
            'created': { '$lt': new Date(new Date() - ms('2d')).toISOString() }
          }]
      }
    },
    {
...
Cleaner.prototype.clearPointers = function (callback) {
  ...
  AggregationCursor(Pointer, query, (pointer, nextPointer) => {
    if (pointer) {
      ...
      Pointer.deleteOne({ _id: pointer._id }, (err) => nextPointer(err, pointer));
    } else {
      nextPointer();
    }
  }, (err) => {
...
Cleaner.prototype.clearInvalidContracts = function (callback) {
  ...
  cursor.on('error', (err) => {
    ...
    async.eachSeries(contracts, (contract, nextContract) => {
      ...
      Contact.findOne({ _id: nodeID, timeoutRate: { $lte: 0.12 } }, (err, contact) => {
        if (err) {
          log.error('Error finding contact', err);

          return nextContract(err);
        }
        if (!contact) {
          log.warn('REMOVE MIRROR/SHARD with hash: %s from contact %s', currentShardHash, nodeID);
          async.parallel([
            next => Shard.updateOne({ _id: this.currentObjectId }, {
              $pull: {
            ...
            next => Mirror.deleteMany({ contact: nodeID }, (err) => {
              log.warn('Finished deleting mirrors for shard %s from contact %s', currentShardHash, nodeID);
              next(err);
            })
          ], () => nextContract());
        } else {
...
Cleaner.prototype.clearMirrorsWithoutContacts = function (callback) {
  ...
  AggregationCursor(Mirror, query, (mirror, nextMirror) => {
    ...
    Mirror.deleteMany({ _id: mirror._id });
    nextMirror(null, mirror);
  }, (err) => callback(err));
};
...
Cleaner.prototype._handleSIGINT = function () {
  ...
  log.info('Received shutdown signal, checking for running cleaner. Current object id: %s', this.currentObjectId);
