...
Cleaner.prototype.clearFramesWithoutBucketEntries = function (callback) {
    ...
    const query = [
        {
            '$match': {
                '$and': [
                    { 'created': { '$gt': new Date(new Date() - ms('14d')) } },
                    { 'created': { '$lt': new Date(new Date() - ms('2d')) } }]
            }
        },
        {
            '$lookup': {
    ...
    AggregationCursor(Frame, query, (data, nextData) => {
        if (data) {
            Frame.findOne({ _id: data._id }, (err, frame) => {
                if (err) {
            ...
            Frame.findOne({ _id: data._id }, (err, frame) => {
                if (err) {
                    ...
                    async.parallel([
                        ...
                        if (err) { log.error('Error deleting frames/pointers: %s', err.message) }
                        // Show must go on
                        nextData(null, data)
                    })
                }
    ...
    const query = [
        {
            '$match': {
                '$and': [
                    ...
                    { 'lastSeen': { '$lte': new Date(new Date() - ms('10d')) } }
                ]
            }
        }
    ];
...
Cleaner.prototype.clearMirrorsWithoutContacts = function (callback) {
    ...
    Mirror.aggregate(query).cursor({ batchSize: 2500 }).exec().toArray().then(mirrors => {
        ...
        async.parallel([
            next => Mirror.deleteMany({ _id: { $in: mirrorIds } }, (err) => next(err)),
            // next => Shard.updateOne({ _id: this.currentObjectId }, { $pull: { contracts: { nodeID: nodeID } } }, (err) => next(err)),
            next => {
                /*
                Shard.updateMany(
