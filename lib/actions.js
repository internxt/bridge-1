...
Actions.prototype.publishNewContractsForShard = function (hash, callback) {
    ...
    Pointer.findOne({ hash: hash }, (err, pointer) => {
        ...
        const contract = new storj.Contract({
            ...
            store_end: Date.now() + ms('10000000d'),
...
Actions.prototype.findShardsInDanger = function (limit, callback) {
    const Mirror = monitor.storage.models.Mirror;

    const query = [
        { $match: { isEstablished: true } },
            ...
            $lookup: {
                ...
                pipeline: [{
                    $match: {
                        $expr: {
                            $and: [
                                { $eq: ["$$mirrorHash", "$shardHash"] },
                                { $eq: ["$isEstablished", false] }]
                        }
                    }
                }],
                as: 'mirrors'
        ...
        { $project: { _id: '$_id', established: '$total', available: { $size: '$mirrors' } } },
        { $sort: { established: 1 } }
    ];

    utils.AggregationCursor(Mirror, query, (data, nextMirror) => {
