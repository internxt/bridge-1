...
class AuditService {
  ...
  // getMirrorsByWallet({ wallet }, dataCallback, errorCallback) {
  ...
  //   AggregationCursor(Mirror, query, dataCallback, errorCallback);
  // }

  getMirrorsByContactAndWallet({ contact, wallet }, dataCallback, errorCallback) {
    const { Mirror } = this._storage.models;
    const query = [
      {
        '$match': {
