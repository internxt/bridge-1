...
class AuditService {
  ...
  getMirrorsByNodeId({ nodeId }) {
    return this._storage.models.Mirror.find({ 'contact': nodeId, 'isEstablished': true });
  }
  ...
  getShardHashByPointerId({ pointerId }) {
    return new Promise((resolve, reject) => {
      ...
      this._storage.models.Pointer.findOne(query, function(err, pointer) {
        ...
        return resolve(pointer && pointer.hash ? pointer.hash : pointer);
      });
    });
  }
  ...
  getShardByShardHash({ shardHash }) {
    return this._storage.models.Shard.findOne({ hash: shardHash });   
  }

  getMirrorByShardHash({ shardHash }) {
    return this._storage.models.Mirror.findOne({ shardHash, isEstablished: true });
  }

  getMirrorByShardHashByNodeId({ shardHash, nodeId }) {
    return this._storage.models.Mirror.findOne({ shardHash, contact: nodeId, isEstablished: true });
  }

  getMirrorsByShardHash({ shardHash }) {
    return this._storage.models.Mirror.find({ shardHash, isEstablished: true });
  }
  ...
  getShardByHashRelatedToNode({ nodeId, shardHash }) {
    return new Promise((resolve, reject) => {
      const query = { 'hash': shardHash, 'trees.nodeID': nodeId };
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = http.get({
        ...
        if (hasResponseFailed) {
          reject(`Request to ${contact.address} failed with HTTP ${res.statusCode}`);
        }
      ...
      request.on('timeout', () => reject(`Request to ${contact.address} failed due to timeout`));
      request.on('abort', () => reject(`Request to ${contact.address} aborted`));
      request.on('error', (err) => reject(`Request to ${contact.address} failed due to ${err.message}`));
      request.end();
    });
  }
