...
class AuditService {
  ...
  getMirrorsByWallet({ wallet }) {
    return new Promise((resolve, reject) => {
      ...
      this._storage.models.Mirror.find(query, function (err, mirrors) {
        if(err) {
          log.error('There was an error querying mirrors!', err);
          reject(err);
        }
        if(mirrors.length === 0) {
          log.info('There are no mirrors for wallet %s', wallet);
        } else {
          log.info('Mirrors found!');
        }
        resolve(mirrors);
      });
    });
