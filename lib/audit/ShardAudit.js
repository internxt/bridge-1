...
class ShardAudit {
  ...
  auditFromMirrors() {
    ...
    return this._service.getContactById({ id: this._shard.contact })
      .then((currentContact) => {
        if (!currentContact) {
          throw new Error('Contact not found');
        }

        contact = storj.Contact(currentContact);
        contract = storj.Contract(this._shard.contract);
      ...
      .then((res) => {
        this.notify(this._shard.contact, res.hash, res.size, res.status, res.reason);
      }).catch((err) => {
        if (err.message === 'Contact not found') {
          return this.notify('Not found', '', 0, false, err.message);
        }
       
        this.notify(this._shard.contact, '', 0, false, err.message);
      });
  }
  ...
  async byHash(shardHash) {
    ...
    const shard = await this._service.getShardByShardHash({ shardHash });

    this._shardFromShards = shard;
    await this.auditFromShardsContracts(); 
  }

  async auditFromShardsContracts() {
    let contract, contact, rawContract, rawContact, token, res, nodeID = '';

    if (this._shardFromShards.contracts && this._shardFromShards.contracts.length > 0) {
      for(const contractWithNode of this._shardFromShards.contracts) {
        try {
          ...
          res = await this.isShardHealthy(contact, contract, token, this._shard.shardHash);
      ...
      log.warn('Shard %s does not have contracts.',  this._shardFromShards.hash);
    }
  }
