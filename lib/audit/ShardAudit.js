...
class ShardAudit {
    ...
    try {
      ...
      if (this._shardHash) {
        if (this._nodeId) {
          ...
          await this._auditByShardByHash();
  ...
  async _auditByShardByHash() {
    const [mirrors, shard] = await Promise.all([
      this._service.getMirrorsByShardHash({ shardHash: this._shardHash }),
      this._service.getShardByShardHash({ shardHash: this._shardHash })
    ]);

    // audit by mirrors
    await mapSeries(mirrors, (mirror, nextItem) => {
      this._shard = mirror;
      this.auditFromMirrors()
        .catch(console.log)
        .finally(() => nextItem());
    });
    ...
    //   return mapSeries(mirrors, (mirror, nextItem) => {
    //     this._shard = mirror;
    //     this.auditFromMirrors()
    //       .catch((err) => {
    //         console.log('err', err);
